...
class Strategy(ABC):
    ...
    def order_target_percent(
        ...
        # CRITICAL FIX: Account for pending orders to prevent duplicate submissions
        # With execution_delay, orders don't fill until next bar. Without this check,
        # strategy would submit the same order repeatedly, causing position accumulation.
        pending_qty = self.broker.get_pending_order_quantity(asset_id)
        effective_qty = current_qty + pending_qty

        # Calculate target quantity
        target_value = portfolio_value * target_pct
        target_qty = target_value / price

        # Calculate DELTA from EFFECTIVE position (includes pending orders)
        delta_qty = target_qty - effective_qty
