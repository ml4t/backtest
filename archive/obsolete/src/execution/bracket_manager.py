...
class BracketOrderManager:
    ...
    @staticmethod
    def get_exit_priority(bracket_type: str) -> int:
        """Get execution priority for bracket exit type.

        Lower number = higher priority (executes first).
        VectorBT priority: SL (1) > TSL (2) > TP (3)
        """
        priorities = {"stop_loss": 1, "trailing_stop": 2, "take_profit": 3}
        return priorities.get(bracket_type, 999)

    def collect_triggered_exits(
        self,
        asset_id: str,
        event,  # MarketEvent
        price: float,
        ...
        trailing_stops: list,
        newly_created: set,
        update_tsl_callback,  # Callable[[Order, event, price], tuple[bool, float]]
        should_trigger_stop_callback,  # Callable[[Order, price], bool]
    ) -> list[tuple[int, str, "Order"]]:
        """Collect all triggered bracket exit orders sorted by priority.

        Returns list of (priority, bracket_type, order) tuples.
        """
        triggered = []

        # Check TP orders (LIMIT in open_orders)
        for order in list(open_orders):
            bracket_type = order.metadata.get("bracket_type")
            if bracket_type == "take_profit":
                if order.order_id in newly_created:
                    continue
                if order.can_fill(price=price, high=event.high, low=event.low):
                    triggered.append((self.get_exit_priority(bracket_type), bracket_type, order))

        # Check SL orders (STOP in stop_orders)
        for order in list(stop_orders):
            bracket_type = order.metadata.get("bracket_type")
            if bracket_type == "stop_loss":
                if order.order_id in newly_created:
                    continue
                if should_trigger_stop_callback(order, price):
                    triggered.append((self.get_exit_priority(bracket_type), bracket_type, order))

        # Check TSL orders
        for order in list(trailing_stops):
            bracket_type = order.metadata.get("bracket_type")
            if bracket_type == "trailing_stop":
                if order.order_id in newly_created:
                    continue
                tsl_triggered, _ = update_tsl_callback(order, event, price)
                if tsl_triggered:
                    triggered.append((self.get_exit_priority(bracket_type), bracket_type, order))
