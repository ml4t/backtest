#!/usr/bin/env bash
# Build script for ml4t.backtest documentation
# Generates both Sphinx HTML docs and consolidated API reference

set -e  # Exit on error

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
SPHINX_DIR="${SCRIPT_DIR}/sphinx"
BUILD_DIR="${SPHINX_DIR}/build"
HTML_DIR="${BUILD_DIR}/html"
VENV_DIR="${PROJECT_ROOT}/.venv"

echo "==================================="
echo "ml4t.backtest Documentation Builder"
echo "==================================="
echo ""
...
if ! command -v sphinx-build &> /dev/null; then
    ...
    echo "Install with: uv pip install sphinx sphinx-rtd-theme sphinx-autodoc-typehints myst-parser"
    exit 1
fi
...
echo "[2/4] Building Sphinx HTML documentation..."
cd "${SPHINX_DIR}"
make html

# Check if build succeeded
if [ ! -d "${HTML_DIR}" ]; then
    echo "ERROR: Sphinx build failed - HTML directory not found"
    exit 1
fi

echo ""
echo "[3/4] Generating consolidated API reference markdown..."

# Create consolidated API reference (manual curation + auto-generated links)
cat > "${SCRIPT_DIR}/api/complete_reference.md" << 'EOF'
...
**Last Generated:** $(date +"%Y-%m-%d %H:%M:%S %Z")
**Build Script:** `docs/build_docs.sh`
**Sphinx Version:** $(sphinx-build --version)
EOF

# Update timestamp in generated file
sed -i "s/\$(date +\"%Y-%m-%d %H:%M:%S %Z\")/$(date +"%Y-%m-%d %H:%M:%S %Z")/g" "${SCRIPT_DIR}/api/complete_reference.md"
sed -i "s/\$(sphinx-build --version)/$(sphinx-build --version)/g" "${SCRIPT_DIR}/api/complete_reference.md"

echo "[4/4] Documentation build complete!"
echo ""
echo "==================================="
echo "Documentation Locations:"
echo "==================================="
echo "HTML Docs:     ${HTML_DIR}/index.html"
echo "API Reference: ${SCRIPT_DIR}/api/complete_reference.md"
echo "Risk API:      ${SCRIPT_DIR}/api/risk_management.md"
echo "Data API:      ${SCRIPT_DIR}/api/data_layer.md"
echo ""
echo "To view HTML docs:"
echo "  open ${HTML_DIR}/index.html"
echo ""
echo "Or start a local server:"
echo "  cd ${HTML_DIR} && python -m http.server 8000"
echo ""
