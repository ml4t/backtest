...
**Issue**: ml4t.backtest shows $0 PnL when holding final position, VectorBT shows $1,603
...
3. Start with Phase 1 MVP:
   - Create `src/ml4t.backtest/portfolio/optimizer.py` (ABC)
   - Create `src/ml4t.backtest/portfolio/simple_optimizers.py`
   - Enhance `src/ml4t.backtest/execution/position_sizer.py`
   - Add `SignalEvent` to `src/ml4t.backtest/core/event.py`
