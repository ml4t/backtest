...
**To**: Next Session - Fix ml4t.backtest phantom trade bug and continue Phase 2 validation
**Context**: 125K/200K tokens (62.5%)
**Project**: ml4t.backtest backtesting library validation
...
**Updated Tests**:
- ✅ Test 1.1: Baseline entries (now uses real data)
- ✅ Test 1.2: Entry/exit pairs (now uses real data)
- ✅ Test 1.3: Multiple round trips (now uses real data)
...
**🔴 CRITICAL BUG DISCOVERED**: Phantom trades when commission enabled!

**Evidence**:
- **WITHOUT fees** (0.0%): Both engines produce 3 trades, perfect match ✅
- **WITH fees** (0.1%): ml4t.backtest produces 4 trades, VectorBT produces 3 trades ❌
...
NO FEES:
  ml4t.backtest: 3 trades, final $100,459.10 ✅
  VectorBT: 3 trades, final $100,459.10 ✅

WITH FEES (0.1%):
  ml4t.backtest: 4 trades, final $99,581.13 ❌ (phantom trade!)
  VectorBT: 3 trades, final $99,858.15 ✅

Phantom Trades:
  Trade 2: ml4t.backtest enters at bar 21 ($28,835.08) instead of bar 60 ($29,044.99)
  Trade 4: ml4t.backtest phantom trade at bar 121 ($29,507.11) - should not exist!
```

**Pattern**: ml4t.backtest enters immediately after exits (bars 20→21, 120→121) **even when there's no entry signal**.

**Verified**: Signal Series is correct - bars 21 and 121 have `entry=False` ✅

### 3. Investigation Progress

**Confirmed Facts**:
1. ✅ Signals are correct (verified with `test_debug_signals.py`)
2. ✅ Phase 1 tests validated - prices match perfectly
3. ✅ Bug only occurs when fees > 0
4. ✅ ml4t.backtestWrapper entry logic checks: `if entry_signal and current_qty == 0:`
5. ✅ Phantom trades occur at bar AFTER exits (N+1 where exit is at bar N)

**Root Cause Hypothesis**:
The bug is in ml4t.backtest's signal processing when `execution_delay=False` and fees are enabled. Possible causes:
...
### Test Status
- ✅ **Phase 1 (Baseline)**: 3/3 tests passing with real data
...
- 🔴 **Phase 2 (Fees)**: 0/3 tests - BLOCKED by phantom trade bug
...
**Context**: Test 2.1 revealed ml4t.backtest generates phantom trades when fees enabled.
...
**Impact**: Must fix ml4t.backtest signal processing before continuing validation roadmap.
...
**Result**: Successfully isolated phantom trade pattern (bars N→N+1 after exits).

## 🚧 Active Challenges

### Challenge 1: Phantom Trade Bug (CRITICAL 🔴)

**Issue**: ml4t.backtest generates phantom entry trades immediately after exits when commission enabled.

**Symptoms**:
- NO fees: 3 trades, perfect match ✅
- WITH fees (0.1%): 4 trades (phantom at bars 21, 121) ❌
- Entry signal at phantom bars is FALSE ✅ (verified)
- Only occurs with fees > 0

**Investigation Status**:
- ✅ Verified signals are correct
- ✅ Confirmed wrapper entry logic looks correct
- ✅ Isolated to fee-related execution path
- ⏳ Need to trace event bus execution flow with fees enabled
- ⏳ Suspect `execution_delay=False` + commission interaction
...
1. Add detailed logging to ml4t.backtestWrapper.SignalStrategy:
   ```python
...
5. Verify fix:
   - Diagnostic test: 3 trades with fees ✅
   - Test 2.1: Final values within $5 ✅
   - All 20 trades at correct bars ✅
...
If phantom trade bug requires deep ml4t.backtest refactoring:

**Option A - Pause validation, fix ml4t.backtest**:
1. Create issue in ml4t.backtest repo documenting bug
2. Fix ml4t.backtest core signal processing
...
3. Create separate work unit for ml4t.backtest fix

**User Decision Required**: Which approach if bug is deep in ml4t.backtest?
...
- `engine_wrappers.py`: Standardized interfaces for ml4t.backtest, VectorBT
...
- Confirms bars 21, 71, 121 have entry=False ✅
...
- `tests/validation/test_1_3_multiple_round_trips.py` ✅ Passing
- `tests/validation/test_2_1_percentage_commission.py` 🔴 Blocked

**Infrastructure**:
- `tests/validation/common/data_generator.py` - Real data loader
- `tests/validation/common/engine_wrappers.py` - ml4t.backtestWrapper (lines 90-290)
...
```
fix: Resolve phantom trade bug in ml4t.backtest signal processing
...
## 🎓 Learnings (For Permanent Memory)
...
**Pattern**: Phase-by-phase validation (baseline → fees → slippage → order types) progressively tests complexity.
...
**Success**: Diagnostic test isolated bug to specific bars (N→N+1 after exits).
...
**Relevant Code**:
- `tests/validation/common/engine_wrappers.py:90-290` - ml4t.backtestWrapper.SignalStrategy
- `src/ml4t.backtest/execution/broker.py` - SimulationBroker
- `src/ml4t.backtest/core/engine.py` - BacktestEngine event loop

## ⚠️ Important Notes

### Critical Finding: Phantom Trade Bug

This bug is BLOCKING all Phase 2+ validation tests. It's not a validation failure - it's a real ml4t.backtest bug that validation successfully discovered.
...
**First Steps**:
1. Add detailed logging to ml4t.backtestWrapper.SignalStrategy.on_market_event
...
**Key Question**: Why does ml4t.backtest enter at bars 21, 71, 121 when `entry_signal=False`?

**Debugging Strategy**: Trace synchronous event execution flow with fees enabled.

---

**Session Complete**: Real data migration successful ✅, Phase 2 blocked by phantom trade bug 🔴
