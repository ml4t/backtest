...
**Issue**: When percentage commission was enabled, ml4t.backtest generated phantom trades (extra trades not in signal series).
...
**Results**:
- ✅ Test 2.1 (0.1% commission) now PASSES perfectly
- ✅ Final values match VectorBT exactly: $97,299.63
- ✅ All 20 trades execute at correct bars
- ✅ No phantom trades
...
**Issue**: ml4t.backtest generates wrong number of trades (14-28 vs expected 20) when fixed fees are enabled.

**Symptoms**:
- Test 2.1 (percentage only): ✅ 20 trades, perfect match
- Test 2.2 (0.1% + $2 fixed): ❌ ml4t.backtest 14-28 trades, VectorBT 20 trades
...
- ml4t.backtest: 28 trades, final $98,160.28
...
**Attempted Fixes**:
1. ✅ Percentage-only entry size: Fixed
2. ❌ Combined fee entry size: `(cash - fixed) / (price * (1 + pct))` - Still has issues
3. ❌ Exit quantity padding (1.01x): Caused negative cash and fewer trades
...
### Phase 2: Transaction Fees (IN PROGRESS ⚠️)
- Test 2.1: Percentage commission (0.1%) - ✅ PASSING
- Test 2.2: Combined fees (0.1% + $2) - ❌ BLOCKED
...
- Bug is complex and may require core ml4t.backtest changes
...
### Challenge 1: Fixed Fee Position Tracking Bug (CRITICAL 🔴)
...
- Should equal: cash
- Formula is algebraically correct ✅
...
1. Create diagnostic test with verbose logging:
   ...
   - Compare ml4t.backtest vs VectorBT position values bar-by-bar
...
4. Verify fix:
   - Test 2.2: 20 trades, final values within $10 ✅
   - Test 2.1 still passes (no regression) ✅
   - All trades at correct signal bars ✅
...
- Integrated VectorBTCommission in ml4t.backtestWrapper
...
**ml4t.backtest Commission Models Available**:
...
**Known Working**:
- Percentage-only fees (0.1%): ✅ Perfect match
- No fees (baseline): ✅ Perfect match
- Real BTC data: ✅ 2.1M bars available
- Signal generation: ✅ Deterministic and correct

**Known Broken**:
- Combined fees (0.1% + $2): ❌ Position tracking mismatch
- Fixed fees alone: ❓ Not tested yet
...
**Example**:
- With 0.1% fee: $100,000 → fill ~3.4537 BTC (error ~0.01%)
- With 0.1% + $2 fee: $99,998 → fill ~3.4551 BTC (error varies with leftover cash)
...
- `src/ml4t.backtest/execution/commission.py:358-417` - VectorBTCommission
- `src/ml4t.backtest/execution/fill_simulator.py:340-428` - Cash constraints
- `src/ml4t.backtest/execution/position_tracker.py:43-94` - Position updates
...
**Session Complete**: TASK-002 ✅ completed, TASK-003 ⚠️ blocked by fixed fee bug
