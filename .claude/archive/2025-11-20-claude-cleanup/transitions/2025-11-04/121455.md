...
Moving validation work from complex crypto_futures project into clean testbed in backtest library (`tests/validation/`). Goal: Validate ml4t.backtest trading mechanics against VectorBT Pro, Backtrader, and Zipline using controlled scenarios with pre-defined signals.
...
2. **Core Implementation**
   - `tests/validation/core/trade.py` - StandardTrade dataclass ✅ TESTED
...
3. **Basic Infrastructure**
   - `tests/validation/scenarios/scenario_001_simple_market_orders.py` - 4 hardcoded signals ✅
   - `tests/validation/runner.py` - Cross-platform executor with SimpleDataFeed ✅
   - ml4t.backtest adapter working (2 trades, $2,972.53 P&L) ✅
   - VectorBT adapter working (2 trades, $36,289.27 P&L) ✅
...
User specifically mentioned crypto_futures project has proper trade reporting built into ml4t.backtest for exactly this purpose.
...
- **Platforms**: ml4t.backtest, VectorBT, Backtrader, Zipline (all 4)
...
- Fixed ml4t.backtest imports (MarketDataType, OrderType from core.types)
...
- **ml4t.backtest**: $2,972.53 (matches manual calculation $2,976.07)
...
- **ml4t.backtest**: Next-bar execution (signal at T → fill at T+1 open)
...
- `extractors/ml4t.backtest.py` - Match BUY/SELL orders into complete trades
...
- `core/__init__.py` + `core/trade.py` - StandardTrade + helpers ✅ TESTED
- `scenarios/scenario_001_simple_market_orders.py` - First test case ✅
...
# Test ml4t.backtest vs VectorBT
...
---------------------------------------------------------------
ml4t.backtest         2        $2,972.53       0.243s     ✅ OK
vectorbt        2        $36,289.27      1.249s     ✅ OK
```
...
**Trade format from ml4t.backtest broker** (`src/ml4t.backtest/execution/broker.py:get_trades()`):
```python
{
    'order_id': str,
    'asset_id': str,
...
**Challenge**: ml4t.backtest returns individual orders, must match BUY→SELL into complete trades.
...
- `tests/validation/adapters/ml4t.backtest_adapter.py` (previous validation work)
...
**First task**: Implement `tests/validation/extractors/ml4t.backtest.py` to convert broker results into List[StandardTrade]
