...
## Active Work
Complete validation of ml4t.backtest backtesting framework against VectorBT Pro, Backtrader, and Zipline.

## Current State

### ✅ VALIDATION COMPLETE
**ml4t.backtest, VectorBT Pro, and Backtrader produce IDENTICAL results** when tested on MA(10,20) crossover strategy on AAPL 2015-2016 data.
...
|-----------|--------|--------|-------------|--------|
| ml4t.backtest | -4.82% | 13 | $9,517.69 | ✅ Reference |
...
**Problem**: Used manual Python MA crossover implementation (120 lines) instead of real ml4t.backtest library.
- Had whipsaw bug causing 25 trades instead of 13
- Didn't validate actual ml4t.backtest execution

**Solution**: Replaced `frameworks/ml4t.backtest_adapter.py` to use `ml4t.backtestWrapper`:
- Now imports real ml4t.backtest via `common/engine_wrappers.py`
...
**Result**: All 6 tests pass, validates real ml4t.backtest library.
...
Also fixed:
- Use pandas `rolling()` for MAs (consistent with ml4t.backtest)
...
**Evidence**:
- ml4t.backtest: BUY on 2015-03-30 @ $126.37
- Backtrader (before): BUY on 2015-03-31 @ $126.09 (one day later)
- Impact: 18% return difference (ml4t.backtest: -4.82%, Backtrader: +13.27%)
...
1. **tests/validation/frameworks/ml4t.backtest_adapter.py** (179 lines)
   - Replaced manual implementation with ml4t.backtestWrapper
   - Fixed data normalization for ml4t.backtest
...
1. `2b5fbd1` - Replace manual ml4t.backtest with real library
...
- ml4t.backtest/VectorBT: 13 trades
...
- ml4t.backtest/VectorBT: Count each order/fill as a "trade"
...
**Recommendation**: Option 3 - document in report, focus on ml4t.backtest/VectorBT/Backtrader agreement.
...
4. **Performance benchmarking** (already noted: VectorBT 0.56s, Backtrader 0.15s, ml4t.backtest 0.97s)
...
**User Expectations**:
- Understand all three frameworks (ml4t.backtest, VectorBT, Backtrader)
...
- `ml4t.backtest_adapter.py` - Uses ml4t.backtestWrapper
...
**ml4t.backtest**: Event-driven, same-bar close execution (default)
...
User asked me to investigate Backtrader discrepancy (was showing 18% difference). I discovered it was executing orders on the next bar instead of same bar. After fixing with `set_coc(True)`, all three frameworks (ml4t.backtest, VectorBT, Backtrader) now produce identical results.
...
**Status**: Validation complete for ml4t.backtest vs VectorBT Pro and Backtrader. Zipline limitation documented.
...
- `tests/validation/common/engine_wrappers.py` - Real ml4t.backtest wrappers
...
3. **Performance optimization**: ml4t.backtest (0.97s) slower than VectorBT (0.56s) and Backtrader (0.15s)
