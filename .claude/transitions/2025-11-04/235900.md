...
- Read `.claude/transitions/2025-11-04/234500.md` which stated:
  ...
  - "qengine uses `close` for entries"  
  - "$7 difference on first trade"
- Implemented "fix" to make qengine use `open` for entries
- Committed changes
...
pf = vbt.Portfolio.from_signals(
    close=ohlcv_adjusted["close"],  # ← Provided
    open=ohlcv_adjusted["open"],    # ← Also provided
...
- qengine entry: $37,625.00 (before "fix")

**Calculate What Each Price Model Would Give**:
- If close = $37,625:
  - close + 0.02% slippage = $37,625 * 1.0002 = $37,632.525 ≈ $37,632.53 ✓
- If open = $37,XXX:
...
2. Identify pattern: When does qengine skip trades?
...
**Key Files**:
- `src/qengine/execution/fill_simulator.py` - Fill price logic (REVERTED)
- `src/qengine/execution/broker.py` - Broker integration (REVERTED)
...
2. **Slippage**: Is 0.02% slippage applied to the fill price? (Seems yes based on $37,625 → $37,632.53)
