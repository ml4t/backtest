...
Implemented VectorBT-compatible exit priority handling in ml4t.backtest's broker where multiple bracket exits (SL, TSL, TP) can trigger in the same bar. Only the highest priority exit fills, with others cancelled via OCO logic.

**Priority Order**: SL (1) > TSL (2) > TP (3)

**Implementation** (`backtest/src/ml4t.backtest/execution/broker.py`):
...
**Key Finding**: ml4t.backtest **ALREADY implements VectorBT entry price logic correctly!**
...
- ⏳ TASK-024: Full ml4t.backtest backtest
...
**Finding**: ml4t.backtest already implements VectorBT entry price logic correctly through previous tasks:
...
**ml4t.backtest Status**: FillSimulator already has `_apply_cash_constraints()` that does this via binary search. Need to verify it matches VectorBT exactly.
...
**Goal**: Run full ml4t.backtest backtest with Q1 2024 data using VectorBT-matching configuration:
...
**Created**:
1. `backtest/src/ml4t.backtest/execution/broker.py` - Exit priority methods (~200 lines added)
...
- `backtest/src/ml4t.backtest/execution/broker.py:376-396` - `_get_exit_priority()`
- `backtest/src/ml4t.backtest/execution/broker.py:398-483` - `_collect_triggered_bracket_exits()`
- `backtest/src/ml4t.backtest/execution/broker.py:749-831` - Priority-based processing

**Entry Price Verification**:
- `backtest/src/ml4t.backtest/execution/fill_simulator.py:177` - Slippage applied first
- `backtest/src/ml4t.backtest/execution/fill_simulator.py:204` - Size uses fill_price
- `backtest/src/ml4t.backtest/execution/fill_simulator.py:211` - Fees use fill_price
