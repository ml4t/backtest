...
**Critical Issue Identified**: test_pytest_integration.py shows QEngine vs Backtrader 18% discrepancy (9.05% std dev)
- QEngine: -4.82% return, 25 trades
- Backtrader: +13.27% return, 12 trades
- Root cause: QEngineAdapter in frameworks/ is a **manual Python implementation**, NOT using real qengine library

### Actual QEngine Validation Status: 9/10 PASSING ✅

**Real validation tests** (test_1_*.py through test_4_*.py) use actual qengine via QEngineWrapper:
...
5. **Floating point tolerance** (src/qengine/portfolio/state.py):
...
### Critical Discovery: Two Different "QEngine" Implementations

**frameworks/qengine_adapter.py** (for test_pytest_integration.py):
- Manual Python implementation of MA crossover
- NO imports from qengine library
...
- Uses REAL qengine library via proper imports
- Imports: `from qengine.engine import BacktestEngine`, etc.
- These are the actual validation tests
- 9/10 passing vs VectorBT Pro

### User Correction on Validation Status

User challenged my assumption: "how do you know this: The real qengine has been validated in the crypto_futures project. It is news to me; validation is still in progress AFAIC"
...
- Uses manual Python implementation, not real qengine
- Shows 18% discrepancy due to whipsaw bug in manual implementation
- Unclear why this test exists when real validation tests (1-4) use actual qengine
- Test file comment says "for CI/CD pipeline" but compares against buggy reference

**Whipsaw Bug Details**:
```python
# QEngineAdapter asymmetric comparisons cause rapid crossovers:
...
**Option 1: Fix QEngineAdapter to use real qengine** (recommended for launch)
- Replace manual implementation with QEngineWrapper
- Actually validates qengine vs Backtrader
- Consistent with other validation tests

**Option 2: Delete test entirely**
- It's redundant with tests 1-4 which validate real qengine
- Manual implementation serves no validation purpose

**Option 3: Clarify as "reference implementation test"**
- Document that it tests a Python reference, not qengine
- Fix the whipsaw bug in the manual implementation
- Keep for CI/CD but don't count as qengine validation
...
5. `src/qengine/portfolio/state.py` - Floating point tolerance (1e-9 → 1e-8)
6. `src/qengine/portfolio/portfolio.py` - Added on_market_event() handler for price updates
7. `src/qengine/engine.py` - Subscribe Portfolio to MARKET events
...
**Key realization**: User is still actively validating qengine - it's NOT "complete" as I mistakenly claimed from reading old docs. The numbered tests (1-4) represent current validation work in progress.
...
2. Why does QEngineAdapter exist as manual implementation vs using real qengine?
