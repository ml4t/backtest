...
**File Modified**: `src/qengine/execution/fill_simulator.py` (lines 456-494)
...
#### Group 1: TSL Trigger Logic (2 failures) 🔴 COMPLEX
...
```
Stage 1: Update peak with open → peak stays 100.0
Stage 2: Calculate TSL level → tsl_level = 99.0 (but DON'T check trigger yet)
Stage 3: Update peak with high → peak becomes 101.0
Stage 4: Recalculate TSL with updated peak → tsl_level = 99.99
         NOW check trigger: low <= tsl_level → 99.5 <= 99.99 → TRIGGERS ✓
```
...
**Verified Working**:
- TSL child order creation: ✅ (trail_percent=1.0, peak_price=100.0, trailing_stop_price=99.0)
- _newly_created_brackets skip logic: ✅ (TSL skipped on creation bar, processed on next bar)
- 4-stage peak tracking: ✅ (updates with both open and high)

**Key Code Locations**:
- `src/qengine/execution/broker.py:494-538` - 4-stage TSL algorithm for bracket exits
- `src/qengine/execution/broker.py:708-745` - 4-stage TSL algorithm for non-bracket trailing stops
- `src/qengine/execution/broker.py:487-488` - Skip newly-created brackets check
- `src/qengine/execution/broker.py:931-933` - Add to _newly_created_brackets set
...
**Code Implementation**: Lines 494-534 in `src/qengine/execution/broker.py`
...
**Chain**:
1. Broker.get_position() → delegates to Portfolio
2. Portfolio.get_position() → delegates to PositionTracker
3. PositionTracker.get_position() → returns `Position | None`
...
print(f"Trigger check: low={event.low} <= tsl={tsl_level} → {can_fill_result}")
```
...
**Key Changes:**
- `src/qengine/execution/fill_simulator.py`: Enhanced _calculate_fill_price() with special stop/TSL handling
