...
**Solution** (src/ml4t.backtest/execution/broker.py, lines 494-534 and 708-745):
```
...
**Files Modified**:
- `src/ml4t.backtest/execution/broker.py:837` - Removed invalid `open` parameter from `try_fill_order()`
- `src/ml4t.backtest/execution/broker.py:853-860` - Fixed `update_position()` to use `quantity_change` instead of `(fill_quantity, side)`

**Pattern**: `quantity_change = fill_quantity if side == BUY else -fill_quantity`

### 3. TSL Peak Tracking Initialization

**File**: `src/ml4t.backtest/execution/bracket_manager.py:185`
...
**Files to Check**:
- `src/ml4t.backtest/execution/fill_simulator.py` - Entry price calculation logic
...
**Files to Check**:
- `src/ml4t.backtest/execution/fill_simulator.py:try_fill_order()` - How stop orders calculate fill price
...
**Code Location**: `src/ml4t.backtest/execution/fill_simulator.py`
...
**Code Location**: `src/ml4t.backtest/execution/fill_simulator.py:try_fill_order()`
...
2. `src/ml4t.backtest/execution/bracket_manager.py`
