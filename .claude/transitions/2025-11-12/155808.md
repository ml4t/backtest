...
**Context**: We successfully completed TASK-008 (limit order validation) for qengine standalone, but when we integrated VectorBT comparison, we hit the **exact architectural flaw** identified in work unit 007's code review.
...
**Test Evidence**:
```
qengine: 14 trades, $101,724.47, final position: -13.66 shares (NEGATIVE!)
...
1. **✅ TASK-008 Completed (qengine standalone)**
   - Added `limit_offset` to BacktestConfig
   - Updated QEngineWrapper for limit orders (entry/exit)
...
- `tests/validation/common/engine_wrappers.py:141-229` - Limit order logic in QEngineWrapper
...
**Observation**: VectorBT filled 4/20 orders, qengine filled 14/20

**Hypotheses**:
- Different limit expiry logic (VectorBT may cancel on signal bar)
- Different intrabar execution (qengine checks all future bars)
...
2. **Keep TASK-008 as qengine-only**
...
- `src/qengine/execution/broker.py:570-627` - Fill processing
- `src/qengine/portfolio/portfolio.py:71` - Error location
- `tests/validation/common/engine_wrappers.py:127-163` - Position queries

**For implementation (if redesigning)**:
- `src/qengine/execution/position_tracker.py` - To be deleted
- `src/qengine/portfolio/portfolio.py` - To be consolidated
- `src/qengine/portfolio/simple.py` - To be merged
...
**Key milestone**: Successfully implemented limit orders in qengine, then discovered the architectural limit preventing VectorBT comparison.
