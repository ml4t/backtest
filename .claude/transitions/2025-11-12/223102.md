...
**Files Modified**:
- `src/ml4t.backtest/data/asset_registry.py` - Added `get_precision_manager()` method
- `src/ml4t.backtest/execution/order.py` - Auto-create default PrecisionManager in `__post_init__()`
- `src/ml4t.backtest/execution/broker.py` - Fixed all `try_fill_order()` calls, added OrderSide import
...
- `src/ml4t.backtest/execution/broker.py:947-950` - `get_position()` now returns float (quantity) instead of Position object
...
- `src/ml4t.backtest/engine.py:163` - Removed `portfolio.initialize()` call (not needed)
- `src/ml4t.backtest/engine.py:211` - Removed `portfolio.finalize()` call (not needed)
- `src/ml4t.backtest/engine.py:234-236` - Updated API: `get_all_positions()`, `returns` property, `get_performance_metrics()`
- `src/ml4t.backtest/engine.py:250` - Use `portfolio.equity` property instead of `get_total_value()`
...
**Removed** (~1,500 lines):
- `src/ml4t.backtest/portfolio/simple.py` (SimplePortfolio)
- `src/ml4t.backtest/portfolio/accounting.py` (PortfolioAccounting)
- 8 legacy test files
...
2. **Fix Trailing Stop Loss Tests** (2-3 hours)
   ...
   - Files: `src/ml4t.backtest/execution/broker.py` (lines 759-809 - trailing stop section)
...
# File: src/ml4t.backtest/execution/broker.py, lines 759-809
...
**Source Code** (8 files):
1. `src/ml4t.backtest/data/asset_registry.py` - Added `get_precision_manager()` to AssetSpec
2. `src/ml4t.backtest/execution/order.py` - Auto-create PrecisionManager in `__post_init__()`
3. `src/ml4t.backtest/execution/broker.py` - Fixed Position API, added OrderSide import, fixed all try_fill_order calls
4. `src/ml4t.backtest/engine.py` - Removed portfolio.initialize/finalize, updated API calls
5. `src/ml4t.backtest/portfolio/portfolio.py` - (no changes, but API confirmed)
