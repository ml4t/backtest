...
1. **✅ Completed Phase 3 (Slippage)**: All 3 tests passing
2. **🔧 Fixed CRITICAL Production Bug**: Position sizing with combined costs
3. **📊 Progress**: 11/19 tasks complete (57.9%)
4. **✅ Full Regression Suite**: 7 validation tests re-runnable anytime
...
- **File**: `src/qengine/execution/order.py:328-335`
...
### Phase Progress
- ✅ **Phase 1** (Baseline): Complete (1 test)
- ✅ **Phase 2** (Transaction Fees): Complete (3 tests)
- ✅ **Phase 2B** (Code Quality): Complete (4 tasks)
- ✅ **Phase 3** (Slippage): Complete (3 tests)
- 🔄 **Phase 4** (Order Types): Started (0 tests complete)
- ⏳ **Phase 5** (Advanced): Not started
- ⏳ **Phase 6** (Stress): Not started
...
#### QEngine Has Full Limit Order Support ✅
- **File**: `src/qengine/execution/fill_simulator.py`
...
**Purpose**: Provide uniform interface for testing multiple engines (qengine, VectorBT, Zipline)

**Problem Solved**:
```python
# Without wrappers - different APIs everywhere
vbt.Portfolio.from_signals(close=..., entries=..., fees=...)
qengine.run(strategy=...)  # Completely different!

# With wrappers - same interface
result1 = QEngineWrapper().run_backtest(ohlcv, entries, exits, config)
...
- `QEngineWrapper`: Converts signals → QEngine Strategy → runs backtest
- `VectorBTWrapper`: Converts signals → VectorBT portfolio
- `ZiplineWrapper`, `BacktraderWrapper`: Other engines

#### What's Needed for TASK-008

**Current Issue**: QEngineWrapper always creates MARKET orders, even when `config.order_type == 'limit'`

**Required Changes**:
1. Add `limit_offset` parameter to `BacktestConfig` dataclass
2. Update `QEngineWrapper.SignalStrategy.on_market_event()` to:
   ```python
   if entry_signal:
...
**Blocker**: Need to add limit order support to QEngineWrapper
...
2. **Update QEngineWrapper Order Creation**
...
4. Run both engines (qengine + VectorBT)
...
### Core Engine
1. `src/qengine/execution/order.py` - Precision fix for fill quantity check
...
2. Modify QEngineWrapper to create LIMIT orders when `config.order_type == 'limit'`
...
**Reference**:
- `src/qengine/execution/fill_simulator.py` (limit order logic)
- `src/qengine/execution/order.py` (Order.can_fill method)
