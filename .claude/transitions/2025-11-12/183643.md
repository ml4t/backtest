...
**File Modified**: `src/ml4t.backtest/execution/fill_simulator.py`
...
1. Stage 1: Update peak with open → peak stays 100.0
2. Stage 2: Calculate TSL → tsl_level = 99.0
3. Stage 3: Update peak with high → **peak becomes 101.0**
4. Stage 4: Recalculate TSL → **tsl_level = 99.99**
5. Check trigger: `low <= tsl_level` → `99.5 <= 99.99` → **TRIGGERS** ✓
...
**Key Code Locations**:
- `src/ml4t.backtest/execution/broker.py:494-538` - 4-stage TSL algorithm
- `src/ml4t.backtest/execution/broker.py:931-933` - newly_created_brackets logic
