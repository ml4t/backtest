...
Test 2.1 revealed that **ml4t.backtest and VectorBT execute trades at different times** when commission is enabled, despite using identical signals.
...
| Trade | ml4t.backtest Entry Time | ml4t.backtest Entry Price | VectorBT Entry Time | VectorBT Entry Price | Entry Time Diff | Price Diff |
|-------|-------------------|---------------------|---------------------|----------------------|-----------------|------------|
...
**Pattern Observed**:
- Trade 2: ml4t.backtest enters at bar 21 (00:21), VectorBT at bar 60 (01:00)
- Trade 4: ml4t.backtest enters at bar 121 (02:01), VectorBT at bar 160 (02:40)
- **ml4t.backtest appears to be re-entering immediately after exit** (bar 20→21, bar 120→121)
...
- **ml4t.backtest**: May be re-entering at bar N+1 even without an explicit entry signal
- **VectorBT**: Only enters when an explicit entry signal exists

This suggests:
1. ml4t.backtest's `SignalStrategy` may have a bug in signal handling
...
- ml4t.backtest might be executing at "open of next bar" while VectorBT executes at "close of signal bar"
...
### Step 3: Inspect ml4t.backtestWrapper Signal Translation

**File**: `tests/validation/common/engine_wrappers.py:58-280`

**Check**:
- How are pandas boolean Series converted to ml4t.backtest signals?
...
5. ⏳ **Fix signal processing**: Correct ml4t.backtest wrapper or SignalStrategy
