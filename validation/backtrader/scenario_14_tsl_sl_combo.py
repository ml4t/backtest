#!/usr/bin/env python3
"""Scenario 14: Trailing Stop + Stop Loss combination validation against Backtrader.
...
Run from .venv-backtrader environment:
    ...
    python validation/backtrader/scenario_14_tsl_sl_combo.py

Success criteria:
- Document Backtrader behavior for TSL vs SL priority
...
def generate_test_data_sl_first(seed: int = 43) -> tuple[pd.DataFrame, np.ndarray]:
    ...
    for i in range(1, n_bars):
        if i == 1:
            # Immediate drop that triggers SL
            change = -0.06
        else:
            change = np.random.randn() * 0.005
        prices.append(prices[-1] * (1 + change))
...
def run_backtrader(prices_df: pd.DataFrame, entries: np.ndarray, scenario: str) -> dict:
    """Run Backtrader with TSL + SL for LONG positions."""
    try:
    ...
    class TSLSLStrategy(bt.Strategy):
        def __init__(self):
        ...
        def next(self):
            if self.bar_count < len(self.entry_signals):
                if self.entry_signals[self.bar_count] and not self.position:
                    # Enter with trailing stop and fixed stop loss
    ...
    cerebro.addstrategy(TSLSLStrategy)
...
def main():
    print("=" * 70)
    print("Scenario 14: TSL + SL Rule Combination (Backtrader)")
    print("=" * 70)
    print(f"\nConfiguration:")
    ...
    print("\n" + "=" * 70)
    if all_match:
        print("✅ TSL + SL PRIORITY MATCHES BACKTRADER")
    else:
