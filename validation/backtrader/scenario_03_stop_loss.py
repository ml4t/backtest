#!/usr/bin/env python3
"""Scenario 03: Stop-Loss validation against Backtrader.

This script validates that ml4t.backtest stop-loss behavior matches Backtrader.

Run from .venv-backtrader environment:
    .venv-backtrader/bin/python3 validation/backtrader/scenario_03_stop_loss.py

Backtrader uses `stop` parameter in bracket orders or explicit StopOrders.

Key differences from VectorBT:
- Backtrader uses next-bar execution by default
- Stop orders are simulated on next bar's OHLC range
- Fill happens at stop price (not necessarily open)

Success criteria:
- Exit trigger timing: Same bar (after stop hit)
- Exit price: At or below stop level
- Final P&L: Within 1%
"""
...
def generate_stop_loss_data(seed: int = 42) -> pd.DataFrame:
    ...
    # Generate OHLCV - opens are slightly higher than close for declining market
    opens = closes + 0.5
    highs = opens + 0.5
    lows = closes - 0.5
...
def run_backtrader(prices_df: pd.DataFrame, sl_pct: float) -> dict:
    """Run backtest using Backtrader with stop-loss."""
    try:
    ...
    class StopLossStrategy(bt.Strategy):
        ...
        def next(self):
            ...
            if len(self) == 1 and not self.position:
                ...
                stop_price = self.entry_price * (1 - self.params.sl_pct)
    ...
    # Add strategy
    cerebro.addstrategy(StopLossStrategy, sl_pct=sl_pct)
    ...
    if strategy.trade_log:
        # Calculate exit price from entry and pnl
...
def run_ml4t_backtest(prices_df: pd.DataFrame, sl_pct: float) -> dict:
    ...
    engine = Engine(
        ...
        stop_fill_mode=StopFillMode.STOP_PRICE,  # Match Backtrader: exact stop price
        stop_level_basis=StopLevelBasis.SIGNAL_PRICE,  # Match Backtrader: stop from signal close
    )
...
def compare_results(bt_results: dict, ml4t_results: dict, sl_pct: float) -> bool:
    """Compare results and report differences."""
    print("\n" + "=" * 70)
    print(f"COMPARISON: Backtrader vs ml4t.backtest (Stop-Loss={sl_pct:.0%})")
    print("=" * 70)
    ...
    print("  Both frameworks: Stop level from signal close price, fill at exact stop")
    print("=" * 70)

    if all_match:
        print("\nVALIDATION PASSED: Stop-loss produces EXACT MATCH with Backtrader")
    else:
...
def main():
    print("=" * 70)
    print("Scenario 03: Stop-Loss Validation (Backtrader)")
    print("=" * 70)
    ...
    prices_df = generate_stop_loss_data()
    ...
    print("\nRunning Backtrader...")
    try:
        bt_results = run_backtrader(prices_df, sl_pct)
    ...
    try:
        ml4t_results = run_ml4t_backtest(prices_df, sl_pct)
    ...
    # Compare results
    success = compare_results(bt_results, ml4t_results, sl_pct)
