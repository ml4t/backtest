#!/usr/bin/env python3
"""Scenario 06: Per-Share Commission validation against Zipline.

Zipline uses PerShare model for per-share commission.

Run from .venv-validation environment:
    source .venv-validation/bin/activate
    python validation/zipline/scenario_06_commission_per_share.py
"""
...
def setup_zipline_bundle(prices_df: pd.DataFrame, bundle_name: str = "test_pershare"):
...
def run_zipline(prices_df: pd.DataFrame, entries: np.ndarray) -> dict:
    ...
    def initialize(context):
        ...
        set_commission(PerShare(cost=PER_SHARE_RATE, min_trade_cost=0.0))
...
def run_ml4t_backtest(prices_df: pd.DataFrame, entries: np.ndarray) -> dict:
    ...
    from ml4t.backtest import (
        DataFeed, Engine, ExecutionMode, NoSlippage, PerShareCommission, Strategy,
    )
