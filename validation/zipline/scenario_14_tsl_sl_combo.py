#!/usr/bin/env python3
"""Scenario 14: Trailing Stop + Stop Loss combination validation against Zipline.

Tests what happens when BOTH TSL and SL conditions can trigger.
Note: Zipline doesn't have built-in TSL or SL, so we implement manually.

Run from .venv-zipline environment:
    source .venv-zipline/bin/activate
    python validation/zipline/scenario_14_tsl_sl_combo.py

Success criteria:
- Document Zipline behavior for TSL vs SL priority
...
def generate_test_data_sl_first(seed: int = 43) -> tuple[pd.DataFrame, np.ndarray]:
    ...
    for i in range(1, n_bars):
        if i == 1:
            change = -0.06  # Immediate drop hits SL
        else:
            change = np.random.randn() * 0.005
        prices.append(prices[-1] * (1 + change))
...
def run_zipline(prices_df: pd.DataFrame, entries: np.ndarray, scenario: str) -> dict:
    """Run Zipline with manual TSL + SL implementation."""
    # State for tracking
    state = {
        "entry_price": None,
...
def main():
    print("=" * 70)
    print("Scenario 14: TSL + SL Rule Combination (Zipline)")
    print("=" * 70)
    print(f"\nConfiguration:")
    ...
    print("\n" + "=" * 70)
    if all_match:
        print("✅ TSL + SL PRIORITY MATCHES ZIPLINE")
    else:
