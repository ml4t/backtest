#!/usr/bin/env python3
"""Scenario 07: Fixed Slippage validation against Zipline.

Zipline supports fixed slippage via FixedSlippage model.

Run from .venv-validation environment:
    source .venv-validation/bin/activate
    python validation/zipline/scenario_07_slippage_fixed.py
"""
...
def setup_zipline_bundle(prices_df: pd.DataFrame, bundle_name: str = "test_slippage"):
...
def run_zipline(prices_df: pd.DataFrame, entries: np.ndarray) -> dict:
    ...
    def initialize(context):
        ...
        set_slippage(FixedSlippage(spread=SLIPPAGE_AMOUNT * 2))  # spread is total, we want per-side
...
def run_ml4t_backtest(prices_df: pd.DataFrame, entries: np.ndarray) -> dict:
    ...
    from ml4t.backtest._validation_imports import (
        DataFeed, Engine, ExecutionMode, FixedSlippage, NoCommission, Strategy,
    )
...
def main():
    ...
    # Compare with tolerance (Zipline has fill timing differences)
    all_match = True
    ...
    all_match &= value_diff_pct < 0.5  # Within 0.5%
