...
def setup_zipline_bundle(prices_df: pd.DataFrame, bundle_name: str = "test_validation"):
    ...
    def make_ingest_func(df):
        def ingest_func(environ, asset_db_writer, minute_bar_writer, daily_bar_writer,
            ...
            if len(trading_df) == 0:
                raise ValueError(f"No trading days found. Data dates: {df_naive.index[:5].tolist()}, "
...
def run_zipline(prices_df: pd.DataFrame, entries: np.ndarray, exits: np.ndarray) -> dict:
    ...
    try:
        ...
        from zipline.api import order, symbol, record, get_datetime, order_target, set_slippage
