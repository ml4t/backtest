#!/usr/bin/env python3
"""Scenario 01: Long-only validation against VectorBT Pro.
...
def run_vectorbt_pro(prices_df: pd.DataFrame, entries: np.ndarray, exits: np.ndarray) -> dict:
    ...
    return {
        ...
        "fill_prices": list(pf.entry_price) if hasattr(pf, "entry_price") else [],
    }
...
def run_ml4t_backtest(prices_df: pd.DataFrame, entries: np.ndarray, exits: np.ndarray) -> dict:
    ...
    return {
        ...
        "fill_prices": [f.price for f in results["fills"]],
    }
...
def compare_results(vbt_results: dict, ml4t_results: dict) -> bool:
    ...
    print("\n" + "=" * 70)
    print("COMPARISON: VectorBT Pro vs ml4t.backtest")
    print("=" * 70)
    ...
    print(f"Final Value: VBT=${vbt_value:,.2f}, ML4T=${ml4t_value:,.2f} (diff={value_pct_diff:.4f}%) {'✅' if values_match else '❌'}")
    ...
    print(f"Total P&L: VBT=${vbt_pnl:,.2f}, ML4T=${ml4t_pnl:,.2f} (diff=${pnl_diff:.2f}) {'✅' if pnl_match else '❌'}")
    all_match &= pnl_match
    ...
    if trades_match and len(vbt_results["trades"]) > 0:
        ...
        for i, (vbt_t, ml4t_t) in enumerate(zip(vbt_trades_list[:5], ml4t_trades_list[:5])):
            vbt_entry = vbt_t.get('Avg Entry Price', vbt_t.get('entry_price', 'N/A'))
            vbt_exit = vbt_t.get('Avg Exit Price', vbt_t.get('exit_price', 'N/A'))
            print(f"  Trade {i+1}: VBT entry={vbt_entry:.2f}, exit={vbt_exit:.2f} | "
...
def main():
    print("=" * 70)
    print("Scenario 01: Long-Only Validation")
    print("=" * 70)
