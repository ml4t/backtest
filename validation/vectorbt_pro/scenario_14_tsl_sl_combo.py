#!/usr/bin/env python3
"""Scenario 14: Trailing Stop + Stop Loss combination validation.

This tests what happens when BOTH TSL and SL conditions can trigger on the same bar.
Critical for understanding rule priority behavior.

Test cases:
1. LONG: TSL triggers before fixed SL is hit
2. LONG: Fixed SL triggers before TSL can engage
3. Which rule takes priority in edge cases?

Run from .venv-vectorbt-pro environment:
    source .venv-vectorbt-pro/bin/activate
    python validation/vectorbt_pro/scenario_14_tsl_sl_combo.py

Success criteria:
- Document VBT Pro behavior for TSL vs SL priority
...
def generate_test_data_tsl_first(seed: int = 42) -> tuple[pd.DataFrame, np.ndarray]:
    ...
    - Price drops to $106.70 (3% from high, triggers TSL)
    - Fixed SL at $95 never hit
    """
    np.random.seed(seed)
    n_bars = 50
    base_price = 100.0
    ...
    for i in range(1, n_bars):
        if i < 10:
            # Rise to $110
            change = 0.01  # 1% per bar
        elif i == 10:
            # Sharp drop that triggers TSL (3% from high ~$110)
            # TSL level = 110 * 0.97 = 106.7
            # This drop should trigger TSL but not SL (which is at $95)
            change = -0.04  # Drop to ~$105.6
        else:
...
def generate_test_data_sl_first(seed: int = 43) -> tuple[pd.DataFrame, np.ndarray]:
    ...
    Scenario:
    - Entry at bar 0 at $100
    - Price drops immediately to $94 (6% drop, hits 5% SL)
    - TSL at 3% from entry $100 would be $97, but SL at $95 triggers first
    """
    np.random.seed(seed)
    n_bars = 50
    base_price = 100.0
    ...
    for i in range(1, n_bars):
        if i == 1:
            # Immediate drop that triggers SL but not TSL from entry
            # SL level = 100 * 0.95 = 95
            # TSL level = 100 * 0.97 = 97 (since no new high yet)
            change = -0.06  # Drop to $94
        else:
            change = np.random.randn() * 0.005
        prices.append(prices[-1] * (1 + change))
...
def generate_test_data_both_breach(seed: int = 44) -> tuple[pd.DataFrame, np.ndarray]:
    ...
    Scenario:
    - Entry at bar 0 at $100
    - Price rises to $104 (new HWM)
    - Sharp drop that breaches both:
      - TSL level = 104 * 0.97 = 100.88
      - SL level = 100 * 0.95 = 95
      - Bar drops from $104 to $94 (breaches both)
    """
    np.random.seed(seed)
    n_bars = 50
    base_price = 100.0
    ...
    for i in range(1, n_bars):
        if i < 5:
            ...
            # Gap down that breaches both levels
            change = -0.10  # Drop from ~104 to ~93.6
        else:
            change = np.random.randn() * 0.005
        prices.append(prices[-1] * (1 + change))
...
def run_vectorbt_pro(prices_df: pd.DataFrame, entries: np.ndarray, scenario: str) -> dict:
    """VBT Pro with TSL + SL for LONG positions."""
    try:
...
def main():
    print("=" * 70)
    print("Scenario 14: TSL + SL Rule Combination")
    print("=" * 70)
    print(f"\nConfiguration:")
    ...
    for name, vbt, ml4t in all_results:
        ...
        if vbt and ml4t:
            ...
            all_match &= match
            print(f"  VBT: bar {vbt_bar}, status {vbt['trades'][0]['status'] if vbt['trades'] else 'N/A'}")
    ...
    print("\n" + "=" * 70)
    if all_match:
        print("✅ TSL vs SL PRIORITY MATCHES VBT PRO")
    else:
        print("⚠️  TSL vs SL PRIORITY DIFFERS - Document behavior in LIMITATIONS.md")
    print("=" * 70)
