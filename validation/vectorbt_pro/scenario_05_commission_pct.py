#!/usr/bin/env python3
"""Scenario 05: Percentage Commission validation against VectorBT Pro.

This script validates that ml4t.backtest commission calculations match VectorBT Pro
...
def generate_test_data(
    ...
    # Generate entry/exit signals: every 10 bars, hold for 5
...
def run_vectorbt_pro(prices_df: pd.DataFrame, entries: np.ndarray, exits: np.ndarray) -> dict:
    """Run backtest using VectorBT Pro with percentage commission."""
    try:
    ...
    pf = vbt.Portfolio.from_signals(
        ...
        size=100,
        size_type="amount",
        fees=COMMISSION_RATE,  # VectorBT uses 'fees' for percentage commission
        slippage=0.0,
        accumulate=False,
        freq="D",
    )
    ...
    # Sum Entry Fees and Exit Fees from trades
    total_fees = 0.0
    if len(trades) > 0:
...
def compare_results(vbt_results: dict, ml4t_results: dict) -> bool:
    ...
    print("\n" + "=" * 70)
    print(f"COMPARISON: VectorBT Pro vs ml4t.backtest (Commission={COMMISSION_RATE:.2%})")
    print("=" * 70)
    ...
    comm_match = comm_diff < 0.01  # Within 1 cent
    print(
    ...
    # Trade-by-trade commission comparison
    if trades_match and len(vbt_results["trades"]) > 0:
        print("\nTrade-by-Trade Commission Comparison:")
        print("-" * 70)
        for i, (vbt_t, ml4t_t) in enumerate(
            zip(vbt_results["trades"][:5], ml4t_results["trades"][:5])
        ):
            vbt_fees = vbt_t.get("Entry Fees", 0) + vbt_t.get("Exit Fees", 0)
            ml4t_fees = ml4t_t.get("commission", 0)
            match = abs(vbt_fees - ml4t_fees) < 0.01
            print(
                f"  Trade {i+1}: VBT fees=${vbt_fees:.2f}, ML4T fees=${ml4t_fees:.2f} "
                f"{'✅' if match else '❌'}"
            )
...
def main():
    ...
    # Calculate expected commission for reference
    expected_trades = entries.sum()
    avg_price = prices_df["close"].mean()
    expected_commission_per_side = 100 * avg_price * COMMISSION_RATE
    print(f"   Expected commission per side: ~${expected_commission_per_side:.2f}")
