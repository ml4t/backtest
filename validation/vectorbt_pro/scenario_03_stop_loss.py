#!/usr/bin/env python3
"""Scenario 03: Stop-Loss validation against VectorBT Pro.

This script validates that ml4t.backtest stop-loss behavior compares to VectorBT Pro.
...
VectorBT Pro uses `sl_stop` parameter for percentage-based stop-loss.
With OHLC data, VBT Pro fills at exact stop price when bar's low <= stop price.

ARCHITECTURAL DIFFERENCE:
- VectorBT Pro (with OHLC): Fills at exact stop price (intrabar simulation)
- ml4t.backtest: Exits at close price when stop threshold breached (bar-based)

Success criteria:
- Exit trigger timing: Same bar
- Exit price: Within bar's OHLC range
- Final P&L: Within 1% (different execution models)
"""
...
def run_vectorbt_pro(prices_df: pd.DataFrame, entries: np.ndarray, sl_pct: float) -> dict:
    """Run backtest using VectorBT Pro with stop-loss and OHLC for realistic fills."""
    try:
    ...
    # When OHLC is provided, VBT Pro checks if stop price falls within [low, high]
...
def run_ml4t_backtest(prices_df: pd.DataFrame, entries: np.ndarray, sl_pct: float) -> dict:
    ...
    engine = Engine(
        ...
        stop_fill_mode=StopFillMode.STOP_PRICE,  # Match VectorBT Pro with OHLC: exact stop price
...
def compare_results(vbt_results: dict, ml4t_results: dict, sl_pct: float) -> bool:
    ...
    print("\n" + "=" * 70)
    print(f"COMPARISON: VectorBT Pro vs ml4t.backtest (Stop-Loss={sl_pct:.0%})")
    print("=" * 70)
    ...
    print("  VBT Pro (OHLC): Fills at exact stop price (intrabar simulation)")
    print("  ml4t.backtest: With STOP_PRICE mode, fills at exact stop price")
    print("=" * 70)

    if all_match:
        print("\nVALIDATION PASSED: Stop-loss produces EXACT MATCH with VectorBT Pro")
    else:
...
def main():
    print("=" * 70)
    print("Scenario 03: Stop-Loss Validation")
    print("=" * 70)
    ...
    try:
        vbt_results = run_vectorbt_pro(prices_df, entries, sl_pct)
