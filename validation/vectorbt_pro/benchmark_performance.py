...
def run_benchmark(n_bars: int, n_assets: int) -> dict:
    ...
    # Calculate speedup
    speedup = vbt_results["runtime_sec"] / ml4t_results["runtime_sec"] if ml4t_results["runtime_sec"] > 0 else 0
...
def main():
    ...
    for n_bars, n_assets in configs:
        ...
        try:
            ...
            print(f"  VBT: {result['vbt_runtime']:.3f}s, {result['vbt_trades']} trades, {result['vbt_memory']:.1f} MB")
            print(f"  ML4T: {result['ml4t_runtime']:.3f}s, {result['ml4t_trades']} trades, {result['ml4t_memory']:.1f} MB")
            print(f"  Speedup: {result['speedup']:.2f}x {'(VBT faster)' if result['speedup'] > 1 else '(ML4T faster)'}")
    ...
    for r in results:
        ...
        print(f"{config:<20} {r['vbt_runtime']:<12.3f} {r['ml4t_runtime']:<12.3f} {speedup_str:<12} {winner}")
