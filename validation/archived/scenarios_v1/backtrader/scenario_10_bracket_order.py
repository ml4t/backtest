#!/usr/bin/env python3
"""Scenario 10: Bracket Order (OCO) validation against Backtrader.

Backtrader supports bracket orders via buy_bracket() method.
Creates main order + stop-loss + take-profit with OCO behavior.

Tests 5% stop-loss and 10% take-profit.

Run from .venv-validation environment:
    source .venv-validation/bin/activate
    python validation/backtrader/scenario_10_bracket_order.py
"""
...
def run_backtrader(prices_df: pd.DataFrame, entries: np.ndarray) -> dict:
    ...
    class BracketStrategy(bt.Strategy):
        ...
        def __init__(self):
            ...
            self.exit_types = {"StopLoss": 0, "TakeProfit": 0}
        ...
        def next(self):
            ...
            if entry and self.position.size == 0 and self.bracket_orders is None:
                current_price = self.data.close[0]
                stop_price = current_price * (1 - STOP_LOSS_PCT)
                limit_price = current_price * (1 + TAKE_PROFIT_PCT)

                # buy_bracket creates main + stop + limit with OCO
        ...
        def notify_order(self, order):
            if order.status == order.Completed:
                if order.issell():
                    # Determine exit type by price comparison
                    if order.executed.price <= self.data.close[0] * (1 - STOP_LOSS_PCT + 0.01):
                        self.exit_types["StopLoss"] += 1
                    else:
                        self.exit_types["TakeProfit"] += 1
                    self.bracket_orders = None
    ...
    cerebro.addstrategy(BracketStrategy, entries=entries)
    ...
    return {
        ...
        "exit_types": strategy.exit_types,
    }
...
def main():
    ...
    print(f"   Exit types: {bt_results['exit_types']}")
    ...
    values_close = value_pct_diff < 2.0  # 2% tolerance for OCO timing differences
    ...
    if trades_match and values_close:
        ...
        print("\nNote: Bracket order fill timing may differ between frameworks.")
    print("=" * 70)
