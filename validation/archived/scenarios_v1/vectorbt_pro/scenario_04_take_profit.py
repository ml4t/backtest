#!/usr/bin/env python3
"""Scenario 04: Take-Profit validation against VectorBT Pro.

This script validates that ml4t.backtest take-profit behavior compares to VectorBT Pro.
...
VectorBT Pro uses `tp_stop` parameter for percentage-based take-profit.
With OHLC data, VBT Pro fills at exact limit price when bar's high >= target.

ARCHITECTURAL DIFFERENCE:
- VectorBT Pro (with OHLC): Fills at exact target price (intrabar simulation)
- ml4t.backtest: Exits at close price when target threshold reached (bar-based)

Success criteria:
- Exit trigger timing: Same bar
- Exit price: Within bar's OHLC range
- Final P&L: Within 5% (different execution models)
"""
...
def run_vectorbt_pro(prices_df: pd.DataFrame, entries: np.ndarray, tp_pct: float) -> dict:
    """Run backtest using VectorBT Pro with take-profit and OHLC for realistic fills."""
    try:
        import vectorbtpro as vbt
    except ImportError:
        raise ImportError("VectorBT Pro not installed. Run in .venv-vectorbt-pro environment.")

    # Run portfolio simulation with take-profit using OHLC for intrabar fills
    # When OHLC is provided, VBT Pro checks if target price falls within [low, high]
    # and fills at exact limit price (more realistic than close-only)
    pf = vbt.Portfolio.from_signals(
        ...
        entries=entries,
        exits=None,  # No explicit exits - rely on take-profit
        init_cash=100_000.0,
        ...
        slippage=0.0,
        accumulate=False,
        tp_stop=tp_pct,  # Take-profit percentage
        freq="D",
    )
...
def run_ml4t_backtest(prices_df: pd.DataFrame, entries: np.ndarray, tp_pct: float) -> dict:
    ...
    class TakeProfitStrategy(Strategy):
        ...
        def on_start(self, broker):
            # Set position-level take-profit rule
        ...
        def on_data(self, timestamp, data, context, broker):
            ...
            # Entry only (exit handled by take-profit rule)
    ...
    engine = Engine(
        ...
        stop_fill_mode=StopFillMode.STOP_PRICE,  # Match VectorBT Pro with OHLC: exact target price
...
def compare_results(vbt_results: dict, ml4t_results: dict, tp_pct: float) -> bool:
    """Compare results and report differences."""
    print("\n" + "=" * 70)
    print(f"COMPARISON: VectorBT Pro vs ml4t.backtest (Take-Profit={tp_pct:.0%})")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    print("CONFIGURATION:")
    print("  Using StopFillMode.STOP_PRICE to match VectorBT Pro with OHLC")
    print("  VBT Pro (OHLC): Fills at exact target price (intrabar simulation)")
    print("  ml4t.backtest: With STOP_PRICE mode, fills at exact target price")
    print("=" * 70)

    if all_match:
        print("\nVALIDATION PASSED: Take-profit produces EXACT MATCH with VectorBT Pro")
    else:
...
def main():
    print("=" * 70)
    print("Scenario 04: Take-Profit Validation")
    print("=" * 70)
    ...
    try:
        vbt_results = run_vectorbt_pro(prices_df, entries, tp_pct)
    ...
    # Compare results
    success = compare_results(vbt_results, ml4t_results, tp_pct)
