#!/usr/bin/env python3
"""Scenario 15: Triple Rule (TSL + TP + SL) combination validation against VectorBT OSS.

Tests what happens when ALL THREE rules are active.

Run from .venv-vectorbt environment:
    source .venv-vectorbt/bin/activate
    python validation/vectorbt_oss/scenario_15_triple_rule.py

Success criteria:
- Document VBT OSS behavior for triple rule priority
...
def generate_test_data_all_breach(seed: int = 45) -> tuple[pd.DataFrame, np.ndarray]:
    ...
    for i in range(1, n_bars):
        if i < 8:
            change = 0.015  # Rise to ~$112 (12% gain, above TP)
        elif i == 8:
            change = -0.16  # Catastrophic drop (breaches all levels)
        else:
            change = np.random.randn() * 0.002
        prices.append(prices[-1] * (1 + change))
...
def run_vectorbt_oss(prices_df: pd.DataFrame, entries: np.ndarray, scenario: str) -> dict:
    """Run VectorBT OSS with TSL + TP + SL."""
    try:
...
def main():
    print("=" * 70)
    print("Scenario 15: Triple Rule (TSL + TP + SL) (VBT OSS)")
    print("=" * 70)
    print(f"\nConfiguration:")
    ...
    for name, gen_func in scenarios:
        ...
        try:
            ...
            if vbt_result['trades']:
                ...
                print(f"  Status: {vbt_result['trades'][0]['status']}")
            else:
                print("  No trades")
    ...
    for name, vbt, ml4t in all_results:
        ...
        if vbt and ml4t:
            ...
            all_match &= match
            print(f"  VBT OSS: bar {vbt_bar}, status {vbt_status}")
    ...
    print("\n" + "=" * 70)
    print("\nEXPECTED BEHAVIOR (based on VBT OSS):")
    ...
    if all_match:
        print("✅ TRIPLE RULE PRIORITY MATCHES VBT OSS")
    else:
