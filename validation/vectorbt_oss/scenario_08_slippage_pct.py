#!/usr/bin/env python3
"""Scenario 08: Percentage Slippage validation against VectorBT OSS.

Tests 0.1% slippage per trade.

Run from .venv-validation environment:
    source .venv-validation/bin/activate
    python validation/vectorbt_oss/scenario_08_slippage_pct.py
"""
...
def run_vectorbt_oss(prices_df, entries, exits):
    ...
    pf = vbt.Portfolio.from_signals(
        ...
        slippage=SLIPPAGE_RATE, fees=0.0, accumulate=False, freq="D",
    )
...
def run_ml4t_backtest(prices_df, entries, exits):
    import polars as pl
    from ml4t.backtest import DataFeed, Engine, ExecutionMode, PercentageSlippage, NoCommission, Strategy
