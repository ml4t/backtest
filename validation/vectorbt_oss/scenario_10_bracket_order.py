#!/usr/bin/env python3
"""Scenario 10: Bracket Order (OCO) validation against VectorBT OSS.
...
Run from .venv-validation environment:
    ...
    python validation/vectorbt_oss/scenario_10_bracket_order.py
"""
...
def run_vectorbt_oss(prices_df, entries):
    ...
    # VectorBT OSS uses sl_stop and tp_stop together for bracket orders
    pf = vbt.Portfolio.from_signals(
        close=prices_df["close"],
        entries=entries,
        exits=np.zeros_like(entries),
        init_cash=100_000.0, size=SHARES_PER_TRADE, size_type="amount",
        sl_stop=STOP_LOSS_PCT,   # Stop-loss
        tp_stop=TAKE_PROFIT_PCT, # Take-profit
...
def run_ml4t_backtest(prices_df, entries):
    ...
    class BracketOrderStrategy(Strategy):
        def on_start(self, broker):
            # Bracket order = StopLoss + TakeProfit in RuleChain
            bracket = RuleChain([
...
def main():
    ...
    print(f"Value diff: {diff_pct:.4f}%")
