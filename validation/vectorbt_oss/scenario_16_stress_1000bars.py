#!/usr/bin/env python3
"""Scenario 16: Stress Test with 1000+ bars against VectorBT OSS.

Tests TSL behavior over extended market conditions.

Run from .venv-vectorbt environment:
    source .venv-vectorbt/bin/activate
    python validation/vectorbt_oss/scenario_16_stress_1000bars.py
"""
...
def main():
    print("=" * 70)
    print(f"Scenario 16: Stress Test with {N_BARS} bars (VBT OSS)")
    print("=" * 70)
    ...
    try:
        vbt_result = run_vectorbt_oss(df, entries)
        print(f"  Trades: {vbt_result['num_trades']}, PnL: ${vbt_result['total_pnl']:.2f}")
    ...
    if vbt_result and ml4t_result:
        ...
        print(f"Trade Count: VBT={vbt_result['num_trades']}, ML4T={ml4t_result['num_trades']} "
