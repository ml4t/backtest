#!/usr/bin/env python3
"""Scenario 04: Take-Profit validation against VectorBT OSS.

This script validates that ml4t.backtest take-profit behavior matches VectorBT OSS.

Run from .venv-vectorbt environment:
    source .venv-vectorbt/bin/activate
    python validation/vectorbt_oss/scenario_04_take_profit.py

VectorBT OSS uses `tp_stop` parameter for percentage-based take-profit.
With OHLC data, VBT OSS fills at exact limit price when bar's high >= target.
...
def run_vectorbt_oss(prices_df: pd.DataFrame, entries: np.ndarray, tp_pct: float) -> dict:
    """Run backtest using VectorBT OSS with take-profit and OHLC for realistic fills."""
    try:
    ...
    # When OHLC is provided, VBT OSS checks if target price falls within [low, high]
...
def run_ml4t_backtest(prices_df: pd.DataFrame, entries: np.ndarray, tp_pct: float) -> dict:
    ...
    engine = Engine(
        ...
        stop_fill_mode=StopFillMode.STOP_PRICE,  # Match VectorBT OSS with OHLC: exact target price
...
def compare_results(vbt_results: dict, ml4t_results: dict, tp_pct: float) -> bool:
    ...
    print("\n" + "=" * 70)
    print(f"COMPARISON: VectorBT OSS vs ml4t.backtest (Take-Profit={tp_pct:.0%})")
    print("=" * 70)
    ...
    print("  VBT OSS (OHLC): Fills at exact target price (intrabar simulation)")
    print("  ml4t.backtest: With STOP_PRICE mode, fills at exact target price")
    print("=" * 70)

    if all_match:
        print("\nVALIDATION PASSED: Take-profit produces EXACT MATCH with VectorBT OSS")
    else:
...
def main():
    print("=" * 70)
    print("Scenario 04: Take-Profit Validation (VectorBT OSS)")
    print("=" * 70)
    ...
    try:
        vbt_results = run_vectorbt_oss(prices_df, entries, tp_pct)
