...
class FundingSettlementLedger:
    ...
    def __init__(self, funding: pl.DataFrame) -> None:
...
def execute_bundle(inputs: Mapping[str, Any]) -> tuple[Any, float, dict[str, int | float]]:
    """Run the engine once and return its native result and engine-only runtime."""
    spec = inputs["spec"]
    config = BacktestConfig.from_dict(spec["backtest_config"])
    targets = _target_map(inputs["targets"])
    rebalance = spec["strategy"]["rebalance"]
    rules = _position_rules(spec["strategy"].get("risk", {}))
    funding = inputs["funding"]
    funding_ledger = FundingSettlementLedger(funding) if funding is not None else None
    ...
    class FrozenTargetStrategy(Strategy):
        def __init__(self) -> None:
            ...
            self._executor = TargetWeightExecutor(
                RebalanceConfig(
                    min_trade_value=float(rebalance["min_trade_value"]),
                    min_weight_change=float(rebalance["min_weight_change"]),
    ...
    feed = DataFeed(prices_df=inputs["market"], feed_spec=config.feed_spec)
...
def main() -> int:
    ...
    result, engine_seconds, funding_metrics = execute_bundle(inputs)
    _write_evidence(
