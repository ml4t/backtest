...
class TestSPYOrderFlowStrategy:
    """Test SPY order flow strategy adapter."""

    def test_spy_order_flow_adapter_creation(self):
        """Test SPY order flow adapter creation."""
        ...
        assert strategy.asset_id == "SPY"

    def test_spy_order_flow_signal_generation(self):
        """Test order flow signal generation."""
        from qengine.strategy.spy_order_flow_adapter import PITData, SPYOrderFlowExternalStrategy
        ...
        # Create PITData with order flow imbalance
        timestamp = datetime(2024, 1, 1, 10, 0, 0)

        # Simulate order flow data with strong buy pressure
        for i in range(10):
            pit_data = PITData(
                ...
                asset_data={
                    "SPY": {
                        "timestamp": timestamp + timedelta(minutes=i),
                        "close": 450.0 + i * 0.5,  # Rising prices
                        "volume": 1000000,
                        "buy_volume": 700000 if i > 5 else 500000,  # Increasing buy pressure
                        "sell_volume": 300000 if i > 5 else 500000,
                    },
                },
                market_prices={"SPY": 450.0 + i * 0.5},
            )

            signal = strategy.generate_signal(timestamp + timedelta(minutes=i), pit_data)

            # Should generate buy signal when imbalance is strong
            if i >= 7:  # After enough data and strong imbalance
                if signal:
                    assert signal.position > 0  # Long signal
                    assert "buy_pressure" in signal.metadata.get("factors", [])

        strategy.on_end()

    def test_spy_order_flow_integration(self):
        """Test full integration with broker mock."""
        from unittest.mock import Mock

        from qengine.strategy.spy_order_flow_adapter import SPYOrderFlowAdapter

        # Create adapter
        strategy = SPYOrderFlowAdapter(
            asset_id="SPY",
            lookback_window=20,
        ...
        # Simulate market events with order flow
        base_time = datetime(2024, 1, 1, 9, 30)

        for i in range(30):
            # Create varying order flow patterns
            if i < 10:
                buy_vol, sell_vol = 500000, 500000  # Balanced
            elif i < 20:
                buy_vol, sell_vol = 800000, 200000  # Strong buy pressure
            else:
                buy_vol, sell_vol = 200000, 800000  # Strong sell pressure

            event = MarketEvent(
                timestamp=base_time + timedelta(minutes=i),
                asset_id="SPY",
                data_type=MarketDataType.BAR,
                close=450.0 + np.sin(i * 0.3) * 2,  # Oscillating price
                volume=buy_vol + sell_vol,
                metadata={"buy_volume": buy_vol, "sell_volume": sell_vol},
            )
