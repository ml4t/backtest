...
from vectorbtpro.signals.nb import first_place_nb, generate_enex_nb, generate_ex_nb, generate_nb
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "entries" in input_names or "entries" in output_names:
            entries_settings = attr_settings.get("entries")
        ...
        if "exits" in output_names:
            exits_settings = attr_settings.get("exits")
        ...
        if "new_entries" in output_names:
            new_entries_settings = attr_settings.get("new_entries")
        ...
        if mode == FactoryMode.Entries:
            ...
            plot.__doc__ = inspect.cleandoc(
                ...
                """.format(Indicator.__name__, "entries")
            )
        else:
            ...
            def plot(
                ...
                **kwargs,
                ...
                if mode == FactoryMode.Exits or mode == FactoryMode.Both:
                    fig = self_col.entries.vbt.signals.plot_as_entries(
                        y=entry_y,
                        trace_kwargs=entry_trace_kwargs,
                        fig=fig,
    ...
    def with_place_func(
        ...
        **kwargs,
        ...
        if entry_place_func_nb is not None:
            ...
            def entry_place_func_nb_prop(
                self, _entry_place_func_nb=entry_place_func_nb
            ) -> tp.Callable:
            ...
            entry_place_func_nb_prop.__qualname__ = (
                f"{Indicator.__name__}.{entry_place_func_nb_prop.__name__}"
            )
            setattr(
                Indicator,
                entry_place_func_nb_prop.__name__,
                class_property(entry_place_func_nb_prop),
            )
        if exit_place_func_nb is not None:

            def exit_place_func_nb_prop(
                self, _exit_place_func_nb=exit_place_func_nb
            ) -> tp.Callable:
            ...
            exit_place_func_nb_prop.__qualname__ = (
                f"{Indicator.__name__}.{exit_place_func_nb_prop.__name__}"
            )
            setattr(
                Indicator, exit_place_func_nb_prop.__name__, class_property(exit_place_func_nb_prop)
            )
        ...
        def _get_func_names(
            func_settings: tp.Kwargs, setting: str, all_names: tp.Sequence[str]
        ) -> tp.List[str]:
        ...
        if mode == FactoryMode.Entries:
            ...
            _1 += ", wait=wait"
            func_str = f"def apply_func({_0}):\n   return generate_func_nb({_1})"
            scope = {
                "generate_func_nb": generate_func_nb,
                "entry_place_func_nb": entry_place_func_nb,
            }
            ...
            func_str = f"def apply_func({_0}):\n   return generate_ex_func_nb({_1})"
            scope = {
                "generate_ex_func_nb": generate_ex_func_nb,
                "exit_place_func_nb": exit_place_func_nb,
            }
            ...
            func_str = f"def apply_func({_0}):\n   return generate_enex_func_nb({_1})"
            scope = {
        ...
        custom_func.__doc__ = custom_func.__doc__.format(
            ...
            if entry_place_func_nb is not None
            else "entry_place_func_nb",
            Indicator.__name__ + ".exit_place_func_nb"
            if exit_place_func_nb is not None
            else "exit_place_func_nb",
        )
