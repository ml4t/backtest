...
class ContinuousFuturesTestCase(
    ...
    def test_current_contract_volume_roll(self):
        ...
        assert contract.symbol == "FOOBARG16", (
            "Auto close at beginning of session. FOOBARG16 is now the current contract."
        )

        bar_data = self.create_bardata(lambda: pd.Timestamp("2016-02-29"))
        contract = bar_data.current(cf_primary, "contract")
        assert contract.symbol == "FOOBARH16", (
    ...
    def test_current_contract_in_algo(self):
        ...
        assert result.primary.symbol == "FOOBARF16", (
            "Primary should be FOOBARF16 on first session."
        )
        assert result.secondary.symbol == "FOOBARG16", (
    ...
    def test_history_sid_session(self):
        ...
        assert window.loc["2016-01-26", cf] == 0, (
            "Should be FOOBARF16 at beginning of window."
        )

        assert window.loc["2016-01-27", cf] == 1, (
            "Should be FOOBARG16 after first roll."
        )

        assert window.loc["2016-02-25", cf] == 1, (
            "Should be FOOBARG16 on session before roll."
        )

        assert window.loc["2016-02-26", cf] == 2, (
            "Should be FOOBARH16 on session with roll."
        )

        assert window.loc["2016-02-29", cf] == 2, (
        ...
        assert window.loc["2016-03-24", cf] == 3, (
            "Should be FOOBARJ16 on session with roll."
        )

        assert window.loc["2016-03-28", cf] == 3, (
    ...
    def test_history_sid_session_delivery_predicate(self):
        ...
        assert window.loc["2016-01-08", cf] == 10, (
    ...
    def test_history_sid_session_secondary(self):
        ...
        assert window.loc["2016-01-26", cf] == 1, (
            "Should be FOOBARG16 at beginning of window."
        )

        assert window.loc["2016-01-27", cf] == 2, (
            "Should be FOOBARH16 after first roll."
        )

        assert window.loc["2016-02-25", cf] == 2, (
            "Should be FOOBARH16 on session before roll."
        )

        assert window.loc["2016-02-26", cf] == 3, (
            "Should be FOOBARJ16 on session with roll."
        )

        assert window.loc["2016-02-29", cf] == 3, (
            "Should be FOOBARJ16 on session after roll."
        )
        ...
        assert window.loc["2016-03-24", cf] == 4, (
            "Should be FOOBARK16 on session with roll."
        )

        assert window.loc["2016-03-28", cf] == 4, (
    ...
    def test_history_sid_session_volume_roll(self):
        ...
        assert window.loc["2016-02-26", cf] == 1, (
            "Should be FOOBARG16 on session before roll."
        )
        ...
        assert window.loc["2016-03-01", cf] == 2, (
            "Should be FOOBARH16 on session after roll."
        )
        ...
        assert window.loc["2016-03-17", cf] == 2, (
    ...
    def test_history_sid_minute(self):
        ...
        assert window.loc[pd.Timestamp("2016-01-26 23:00", tz="UTC"), cf.sid] == 0, (
            "Should be FOOBARF16 on on minute before roll minute."
        )

        assert window.loc[pd.Timestamp("2016-01-26 23:01", tz="UTC"), cf.sid] == 1, (
        ...
        assert window.loc[pd.Timestamp("2016-01-27 22:32", tz="UTC"), cf.sid] == 1, (
            "Should be FOOBARG16 at beginning of window."
        )

        assert window.loc[pd.Timestamp("2016-01-27 23:01", tz="UTC"), cf.sid] == 1, (
    ...
    def test_history_close_minute(self):
        ...
        assert window.loc["2016-02-26 22:32", cf] == 125241.412, (
            "Should be FOOBARH16 at beginning of window."
        )

        assert window.loc["2016-02-28 23:01", cf] == 125250.001, (
    ...
    def test_history_close_minute_adjusted(self):
        ...
        assert window.loc["2016-02-25 23:01", cf_mul] == 125240.001, (
            "Should be FOOBARH16 on minute after roll, unadjusted."
        )

        assert window.loc["2016-02-25 23:01", cf_add] == 125240.001, (
            "Should be FOOBARH16 on minute after roll, unadjusted."
        )
        ...
        assert window.loc["2016-02-26 22:32", cf_mul] == 125241.412, (
            "Should be FOOBARH16 at beginning of window."
        )

        assert window.loc["2016-02-28 23:01", cf_mul] == 125250.001, (
            "Should remain FOOBARH16 on next session."
        )
    ...
    def test_history_close_minute_adjusted_volume_roll(self):
        ...
        assert window.loc["2016-02-28 23:01", cf_add] == 125250.001, (
            "Should be FOOBARH16 on minute after roll, unadjusted."
        )
        ...
        # No adjustments in this window.
        assert window.loc["2016-02-29 22:32", cf_mul] == 125251.412, (
            "Should be FOOBARH16 at beginning of window."
        )

        assert window.loc["2016-02-29 23:01", cf_mul] == 125260.001, (
            "Should remain FOOBARH16 on next session."
        )
...
class TestOrderedContracts:
    def test_contract_at_offset(self):
        ...
        assert oc.contract_at_offset(1, 0, start_dates[-1].value) == 1, (
            "Offset of 0 should return provided sid"
        )

        assert oc.contract_at_offset(1, 1, start_dates[-1].value) == 2, (
            "Offset of 1 should return next sid in chain."
        )

        assert None is oc.contract_at_offset(4, 1, start_dates[-1].value), (
    ...
    def test_active_chain(self):
        ...
        assert list(chain) == [1, 2], "[1, 2] should be the active contracts on 01-02."
        ...
        assert list(chain) == [4], "[4] should be active beginning at its start date."
