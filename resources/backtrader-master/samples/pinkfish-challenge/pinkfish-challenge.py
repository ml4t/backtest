...
class St(bt.Strategy):
    ...
    def notify_order(self, order):
        if order.isbuy() and order.status == order.Completed:
            print(
                "-- BUY Completed on:", self.data.num2date(order.executed.dt).strftime("%Y-%m-%d")
            )
...
def parse_args(pargs=None):
    ...
    parser.add_argument(
        "--no-replay", required=False, action="store_true", help=("Use Replay + replay filter")
    )

    parser.add_argument(
        "--market", required=False, action="store_true", help=("Use Market exec instead of Close")
    )
