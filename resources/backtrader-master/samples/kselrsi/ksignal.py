...
class TheStrategy(bt.SignalStrategy):
    ...
    def notify_order(self, order):
        ...
        if order.status == order.Completed:
            print(
                "%s: Size: %d @ Price %f"
                % ("buy" if order.isbuy() else "sell", order.executed.size, order.executed.price)
            )
    ...
    def __init__(self):
        ...
        rsi = bt.indicators.RSI(
            period=self.p.rsi_per, upperband=self.p.rsi_upper, lowerband=self.p.rsi_lower
        )
...
def parse_args(pargs=None):
    ...
    parser.add_argument(
        "--fromdate", required=False, default="2012-09-01", help="Ending date in YYYY-MM-DD format"
    )

    parser.add_argument(
        "--todate", required=False, default="2016-01-01", help="Ending date in YYYY-MM-DD format"
    )
