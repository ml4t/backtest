...
def checkdate(dt, d):
    ...
    Y = decade + YCode
    if dt.year > Y:  # Example: year 2019 ... YCode is 0 for 2023
        Y += 10
...
def parse_args(pargs=None):
    ...
    parser.add_argument(
        "--no-cerebro", required=False, action="store_true", help="Use RollOver Directly"
    )

    parser.add_argument("--rollover", required=False, action="store_true")

    parser.add_argument(
        "--checkdate", required=False, action="store_true", help="Change during expiration week"
    )
