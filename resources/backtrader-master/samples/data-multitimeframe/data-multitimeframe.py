...
def runstrat():
        ...
        if args.oldrs:
            if args.replay:
                data2 = bt.DataReplayer(
                    dataname=data, timeframe=tframes[args.timeframe], compression=args.compression
                )
            else:
    ...
    # Run over everything
    cerebro.run(
        runonce=not args.runnext, preload=not args.nopreload, oldsync=args.oldsync, stdstats=False
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--noresample", action="store_true", help="Do not resample, rather load larger timeframe"
    )
    ...
    parser.add_argument(
        "--compression", default=1, required=False, type=int, help="Compress n bars into 1"
    )

    parser.add_argument(
        "--indicators", action="store_true", help="Wether to apply Strategy with indicators"
    )

    parser.add_argument(
        "--onlydaily", action="store_true", help="Indicator only to be applied to daily timeframe"
    )

    parser.add_argument(
        "--period", default=10, required=False, type=int, help="Period to apply to indicator"
    )
