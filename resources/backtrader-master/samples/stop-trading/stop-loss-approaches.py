    ...
    def next(self):
        if not self.position and self.crossup > 0:
            ...
            if not self.p.trail:
                ...
                self.sell(
                    exectype=bt.Order.StopTrail, trailamount=self.p.trail, parent=self.buy_order
                )
...
def parse_args(pargs=None):
    parser = argparse.ArgumentParser(
        formatter_class=argparse.ArgumentDefaultsHelpFormatter, description=("Stop-Loss Approaches")
    )
