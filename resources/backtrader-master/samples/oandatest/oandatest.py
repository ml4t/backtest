        ...
        if self.datastatus and not self.position and len(self.orderid) < 1:
            if not self.p.usebracket:
                if not self.p.sell:
                    ...
                    self.order = self.buy(
                        size=self.p.stake, exectype=self.p.exectype, price=price, valid=self.p.valid
                    )
                else:
            ...
            if self.order is None:
                if not self.p.sell:
                    self.order = self.sell(
                        size=self.p.stake // 2, exectype=bt.Order.Market, price=self.data0.close[0]
                    )
                else:
                    self.order = self.buy(
                        size=self.p.stake // 2, exectype=bt.Order.Market, price=self.data0.close[0]
...
def parse_args(pargs=None):
    parser = argparse.ArgumentParser(
        formatter_class=argparse.ArgumentDefaultsHelpFormatter, description="Test Oanda integration"
    )
    ...
    parser.add_argument(
        "--no-store", required=False, action="store_true", help="Do not use the store pattern"
    )

    parser.add_argument(
        "--debug", required=False, action="store_true", help="Display all info received from source"
    )

    parser.add_argument(
        "--token", default=None, required=True, action="store", help="Access token to use"
    )

    parser.add_argument(
        "--account", default=None, required=True, action="store", help="Account identifier to use"
    )
    ...
    parser.add_argument(
        "--data0", default=None, required=True, action="store", help="data 0 into the system"
    )

    parser.add_argument(
        "--data1", default=None, required=False, action="store", help="data 1 into the system"
    )
    ...
    parser.add_argument(
        "--historical", required=False, action="store_true", help="do only historical download"
    )
    ...
    pgroup.add_argument(
        "--replay", required=False, action="store_true", help="replay to chosen timeframe"
    )

    pgroup.add_argument(
        "--resample", required=False, action="store_true", help="resample to chosen timeframe"
    )
    ...
    parser.add_argument(
        "--no-bar2edge", required=False, action="store_true", help="no bar2edge for resample/replay"
    )
    ...
    parser.add_argument(
        "--trade", required=False, action="store_true", help="Do Sample Buy/Sell operations"
    )
    ...
    parser.add_argument(
        ...
        action="store",
        help=(
            "Cancel a buy order after n bars in operation, to be combined with orders like Limit"
        ),
    )
