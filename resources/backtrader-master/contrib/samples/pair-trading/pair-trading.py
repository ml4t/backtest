...
class PairTradingStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if (self.zscore[0] > self.upper_limit) and (self.status != 1):
            ...
            self.log(
                "BUY CREATE %s, price = %.2f, qty = %d" % ("KO", self.data1.close[0], y + self.qty2)
            )
            self.buy(
...
def parse_args():
    ...
    parser.add_argument(
        "--data0", "-d0", default="../../datas/daily-PEP.csv", help="1st data into the system"
    )

    parser.add_argument(
        "--data1", "-d1", default="../../datas/daily-KO.csv", help="2nd data into the system"
    )

    parser.add_argument(
        "--fromdate", "-f", default="1997-01-01", help="Starting date in YYYY-MM-DD format"
    )

    parser.add_argument(
        "--todate", "-t", default="1998-06-01", help="Starting date in YYYY-MM-DD format"
    )

    parser.add_argument(
        "--period", default=10, type=int, help="Period to apply to the Simple Moving Average"
    )
    ...
    parser.add_argument(
        "--commperc", default=0.005, type=float, help="Percentage commission (0.005 is 0.5%%"
    )
